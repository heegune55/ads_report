...
top    = [a for a in ads_sorted if a["roas"] and a["roas"] >= 2.0]
middle = [a for a in ads_sorted if a["roas"] and 1.8 <= a["roas"] < 2.0]
...
print(f"  -> ROAS 200%+: {len(top)}개 / ROAS 180~200%: {len(middle)}개")
...
blocks = [
    h1("🎯 KB 프로모션 소재 성과 리스트 (2025~2026)"),
    co(
        f"분석 기간: {period}  |  조건: 지출 ₩1,000,000↑  |  "
        f"200%+: {len(top)}개  180~200%: {len(middle)}개  |  블렌드 ROAS: {blend_roas:.2f}x  |  총 구매: {total_pur:.0f}건",
        "📌"
    ),
    div(),
]
...
if top:
    blocks.append(h2(f"✅ ROAS 200% 이상 ({len(top)}개)"))
    blocks.append(tbl(HEADERS, make_rows(top)))
    blocks.append(div())

if middle:
    blocks.append(h2(f"🟡 ROAS 180~200% ({len(middle)}개)"))
    blocks.append(tbl(HEADERS, make_rows(middle)))
    blocks.append(div())
